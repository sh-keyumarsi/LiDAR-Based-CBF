...
class Unicycle(Dynamics): # UNTESTED
    ...
    def set_input(self, input, key="u", check_input=True, max_lin=None, max_omg=None): 
        # NOTE this part is beneficial but checking this each time can take some time for the simulation.
        self.input[key] = ensure_xyz(input, '[SingleIntegrator input_vel]') if check_input else input
        ...
        v_lin, omega = current_input[0], current_input[1]
        # print( 'before:' + '{:.2f}'.format(v_lin) + 'and' + '{:.2f}'.format(omega) )
        if max_lin is not None: 
            v_lin = v_lin if abs(v_lin) < max_lin else max_lin*v_lin/abs(v_lin)
        if max_omg is not None: 
            omega = omega if abs(omega) < max_omg else max_omg*omega/abs(omega)
        # print( 'after:' + '{:.2f}'.format(v_lin) + 'and' + '{:.2f}'.format(omega) )

        self.input["V"] = v_lin
        self.input["omg"] = omega
